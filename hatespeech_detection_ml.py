...
# Sample words to test the model
test_words = ['rafiki', 'mjinga']  # 'rafiki' is non-offensive, 'mjinga' is offensive

# Preprocess the test words
test_words_processed = [preprocess_text(word) for word in test_words]

# Convert test words to feature vectors using the same CountVectorizer
test_vectorized = vectorizer.transform(test_words_processed)

# Make predictions
predictions = log_reg_model.predict(test_vectorized)

# Create a DataFrame for the results
results = pd.DataFrame({'Word': test_words, 'Prediction': predictions})
